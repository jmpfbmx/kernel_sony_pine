...
int retry_count = 10;
retry:
wait_nv_flg = atomic_read(&wait_nv);
CCCI_UTIL_ERR_MSG("[YC] Wite TA Restore , wait_nv_flg =  %d\n", wait_nv_flg);
CCCI_UTIL_ERR_MSG("[YC] Wite TA Restore , retry_count =  %d\n", retry_count);
if(wait_nv_flg == 0 && retry_count != 0)
{
retry_count--;
msleep(500);
goto retry;
}
atomic_set(&wait_nv, 1);
CCCI_UTIL_ERR_MSG("[YC] Wite TA Restore , bye bye\n");
#endif
//<2016/11/09-ychuang,
...
//<2016/11/09-ychuang,[S1] Check TA restore status before starting the modem.
#if defined(Wait_TA_Restore)
static ssize_t wait_nv_store(const char *buf, size_t count)
{
CCCI_UTIL_ERR_MSG("[YC] wait_nv_store.\n");
atomic_set(&wait_nv, 1);
if(atomic_read(&wait_nv)==1)
return 0;
return -1;
}
#endif
CCCI_ATTR(nv_ready, 0660, NULL, &wait_nv_store);
//<2016/11/09-ychuang,
